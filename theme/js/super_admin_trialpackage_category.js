...
$('#save,#update').on("click", function (e) {
  ...
  if (confirm("Do You Wants To Save Or Update Record ?")) {
    e.preventDefault();
    data = new FormData($('#trialpack-category-form')[0]);//form name
    /*Check XSS Code*/
    ...
    $.ajax({
      type: 'POST',
      url: base_url + 'super_admin/create_trial_pack_category',
      data: data,
      cache: false,
      contentType: false,
      processData: false,
      success: function (result) {
        ...
        if (result == "success") {
          ...
          window.location.replace(base_url+'super_admin/trial_pack_category');
        }
...
function update_status(id, status) {
  var base_url = $("#base_url").val();
  $.post(base_url + "super_admin/trial_pack_category_status_update", { id: id, status: status }, function (result) {
    //alert(result);return;
    if (result == "success") {
      toastr["success"]("Record Updated Successfully!");

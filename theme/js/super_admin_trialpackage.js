...
$('#save,#update').on("click", function (e) {
  ...
  check_field("trial_pack_catid");
	check_field("day_or_month");
	check_field("name");
	check_field("days");
  if (flag == false) {
  ...
  if (confirm("Do You Wants To Save Or Update Record ?")) {
    e.preventDefault();
    data = new FormData($('#trial-package-form')[0]);//form name
    /*Check XSS Code*/
    ...
    $.ajax({
      type: 'POST',
      url: base_url + 'super_admin/create_trial_package',
      data: data,
      cache: false,
      contentType: false,
      processData: false,
      success: function (result) {
        ...
        if (result == "success") {
          ...
          window.location.replace(base_url+'super_admin/trial_package');
        }
...
function update_status(id, status) {
  var base_url = $("#base_url").val();
  $.post(base_url + "super_admin/trial_package_status_update", { id: id, status: status }, function (result) {
    //alert(result);return;
    if (result == "success") {
      toastr["success"]("Record Updated Successfully!");
...
function update_primary_status(id,cat_id,status) {
  var base_url = $("#base_url").val();
  $.post(base_url + "super_admin/trial_package_primary_status_update", { id: id,cat_id:cat_id }, function (result) {
    //alert(result);return;
    if (result == "success") {
      toastr["success"]("Record Updated Successfully!");
      success.currentTime = 0;
